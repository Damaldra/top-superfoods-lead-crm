...
- `localhost:5173` — **лендінг**: заповнити форму, надіслати заявку;
- `localhost:5174` — **панель менеджера**, вхід `admin` / `admin`: дашборд (KPI), заявки (пошук/фільтр/сортування/CSV), повтор доставки, особистий кабінет;
- `localhost:4000` — **REST API** (бекенд; напряму у браузері відкривати не треба).
